...
class GroundTruthNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('real_width', 0.60)
        self.declare_parameter('real_height', 0.47)
        ...
        # Carregar calibração da câmera (se fornecida)
        self.cam_mat = None
    ...
    def process_frame(self):
        ...
        # 1. Undistort (remover distorção da lente se calibração existir)
        if self.cam_mat is not None and self.dist_coeff is not None:
            frame = cv2.undistort(frame, self.cam_mat, self.dist_coeff)
