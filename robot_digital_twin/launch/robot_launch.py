...
def generate_launch_description():
    ...
    odom_node = Node(
        ...
        name='odometry_node',
        output='screen',
        # O parâmetro no Python foi nomeado w_imu na Tarefa 2
        parameters=[{'w_imu': imu_weight}] 
    )

    # Nó 3: Controlador (Missões e Manual)
    # Nota: Em sistemas ROS, ler teclado no Launch exige prefixos de terminal interativo.
    controller_node = Node(
        ...
        output='screen',
        emulate_tty=True,
        prefix="xterm -e" # Abre um terminal Xterm separado para você conseguir digitar WASD limpo
    )
    ...
    # Nó 6: Robot State Publisher (Nó nativo do ROS)
    rsp_node = Node(
    ...
    # Nó 7: RViz2 (Nó nativo, com condicional IfCondition)
    rviz_node = Node(
        package='rviz2',
        executable='rviz2',
        name='rviz2',
